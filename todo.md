...
The server now emits a `BsLiveRules` event (`"bslive rules violated"`) on startup for watch-based configs. The test
utility code doesn't recognise this event kind and logs `"cannot continue - probably an unsupported type"`, causing
`watch-graceful` to timeout waiting for expected messages.
...
Both issues stem from changes to task execution on the `defaults` branch. To fix: either revert the output format
changes (prefix/task_id) or update the test expectations and utils to handle the new shape.
