    ...
    reloadStylesheet(path) {
        ...
        if (match) {
            ...
            if (options.reloadMissingCSS) {
                this.console.debug(`will reload all stylesheets because path '${path}' did not match any specific one. \
To disable this behavior, set 'options.reloadMissingCSS' to 'false'.`
                ...
                this.console.debug(`will not reload path '${path}' because the stylesheet was not found on the page \
    ...
    generateCacheBustUrl(url, expando) {
        ...
        if (options.overrideURL) {
            if (url.indexOf(options.serverURL) < 0) {
                ...
                this.console.debug(`is overriding source URL ${originalUrl} with ${url}`);
            }
        }
